...
(defcustom consult-omni-projects-default-new-func #'consult-omni--projects-make-new-project-dwim
...
(defun consult-omni--projects-add-project-to-list (cand)
  ...
  (let* ((dir (read-directory-name (concat "Find Project Directory for " (propertize (format "%s: " cand) 'face 'font-lock-keyword-face)) default-directory nil t))
...
(defun consult-omni--projects-make-new-project (cand)
  ...
  (let* ((dir (read-directory-name (concat "Select Parent Directory for " (propertize (format "%s: " cand) 'face 'font-lock-keyword-face)) default-directory nil nil))
        (name (read-string (concat "Project Name for " (propertize (format "%s: " cand) 'face 'font-lock-keyword-face)) cand))
        (target (expand-file-name (concat name "/") dir)))
...
(defun consult-omni--projects-make-new-project-dwim (cand)
 "Add existing ro make new project for CAND."
  (let* ((dir (read-directory-name (concat "Select Directory for" (propertize (format "%s: " cand) 'face 'font-lock-keyword-face)) default-directory nil t))
         (project (project--find-in-directory dir))
         (root (and project (project-root project)))
         (existing  (if project
      ...
      (existing
       (when (file-exists-p dir) (project--remember-dir dir)
             (if root (project-switch-project root)
               (dired dir))))
      (new
       (let* ((name (read-string (concat "Project Name for " (propertize (format "%s: " cand) 'face 'font-lock-keyword-face)) cand))
        (target (expand-file-name (concat name "/") dir)))
       (funcall consult-omni-projects-create-new-func target)
