      ...
      (lambda (response _)
        (when response
          (let ((decorated
            ...
            (setq output (propertize decorated
                        :title response
                        :source "gptel"
                        :url nil
                        :query query
...
metadata so it can be send to `gptel'."
  ...
  (when-let ((title (or (and (stringp consult-omni-gptel-cand-title) consult-omni-gptel-cand-title)
                   (and (functionp consult-omni-gptel-cand-title)
                        (funcall consult-omni-gptel-cand-title input args)))))
    (when callback
      (funcall callback (list title)))
    (list title)
    ))
