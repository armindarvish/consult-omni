...
consult-omni is a successor of [[https://github.com/armindarvish/consult-web][consult-web]], with expanded features and functionalities.
...
For each module (a.k.a each source) consult-omni provides one or two interactive command. A dynamic command and a static command. For example if =consult-omnni-wikipedia= is loaded, then you may will get =consult-omni-wkipedia= and/or =consult-omni-wikipedia-static=, depending on your settings (customization variables). The difference between the dynamic and static commands is in how consult-omni interacts with the user.

- =static commands=: In static commands (the ones with =-static= suffix in their name), the user is first asked to provide a search term and then a list of candidates are fetched for that search term from the source. This list of candidates is static and cannot chnage, hoewver narrowing down is still possible similar to any other minibuffer completion. Here is a screenshot for wikipedia:
#+ATTR_ORG: :width 800px
#+ATTR_LATEX: :width 800px
#+ATTR_HTML: :width 800px
...
- =dynamic commands=: Dynamic commands fetch the result dynamically based on the user's minibuffer input, meaning that the results get updated as the user types and if the user changes the input, the list of results get updated. Here further narrow down is still possible by using consult's async syntax (see ). Here is a screenshot
...
By default consult-omni provides two main inteactive commands.
...
Note that unlike the predecessor consult-web, cosult-omni does not provide other multi-source interactive commands (e.g. scholar search) anymore. It's now up to the user to define such commands for their custom workflow. For example one user may define separate interactive commands to query web sources, v.s. local sources, while another user may want to define interactive commands for knowledge sources (acadmeic references, notes, ...) v.s. utilities (apps, buffers, files,...). Examples for how to define new interacitve commands are provided below in the advanced config [[id:DFBCF797-44D6-4265-B737-77916700C43D][here]].
...
For narrowing down the results you need adding =#= (or another charatcer defined in  =consult-async-split-style=) after the search query. For example typing the following in the minibuffer:
#+begin_example
...
When using consult, we deal with differnet types of sources, either elisp functions (e.g. =buffer-list=, =re-search-forward=) or commandline programs (e.g. =grep=, =notmuch=, =gh=, =mu=, ...). They return a list of candidates, which in turn are passed to =completing--read=. While consult provides a way to combine multiple sources with =consult--multi=, the ability to combine sources of different types (commandline program with elisp for example) is limited. In consult-omni we solve this by creating some wrappers and machinery around built-in consult functions and by having each source declare its type. This type  tells consult-omni how to collect candidates from that source and combine with other sources. The type can be ='sync=, ='dynamic=, or ='async=.

- ='sync=: This is a synchronous source, meaning that when consult-omni calls the request function to collect candidates from this source, it calls it synchronously (blocks the emacs pocess) and waits for a returned value (expecting a list of candidates or nil). This is for example suitbale for sources that simply run an elisp function and return a list quickly like =buffer-list=. There is almost no gain in calling this function asynchronously because the overhead for async control (timers and watchers,...) is probably going to cost more time than just calling the function directly and synchronously.

- ='dynamic=: Dynamic sources are those that would benefit or require calling the colecting elisp function asynchronously. For example, if colelcting items from a source requires to send an http request to a server and waiting for the response, then a dynamic type should be used, so that emacs process is not blocked while waiting for the response. In this case the request function for the source (the function that returns the items) should take a callback function that will be called when the response arrives. This is for example is suitable for getting resopnse from web search APIs like Google's, Brave's or OpenAI's APIs.
...
*** Modular
You can only load the parts you need. For example if all you need is an autosuggestion utility similar to =helm-google-autosuggest=, then you can use aminimal config like this:
...
Here is a list of current sources. For details on how to set and use each source, refer to the [[https://github.com/armindarvish/consult-omni/wiki][wiki pages]].
...
In addition to these settings, some sources have their own customization variables, etc. which is discussed in the [[https://github.com/armindarvish/consult-omni/wiki][wiki pages]].
...
List of modules to load. By setting this you before calling =consult-omni-sources-load-modules=, you can limit the modules (sources) that are loaded and can avoid errors due to mising requirements for certain sources. For example for just loading Wikipedia:
#+begin_src emacs-lisp
...
This variable a stores a function that is called for formating the candidates (on any source that does not have an explivit formating function). consult-omni provides two example functions:
...
This variable controls which backend is used by consult-omni for http requests. consult-omni supports 3 different backends:
...
For example to change the backend to ='plz=, you can do:
#+begin_src emacs-lisp
...
#+end_src
*** A config with mostly default Settings
#+begin_src emacs-lisp
(use-package consult-omni
  :straight (consult-omni :type git :host github :repo "armindarvish/consult-omni" :branch "main" :files (:defaults "sources/*.el"))
  :after consult
...
- Alfred: Alfred does provide more customizability and advanced workflows, but that does not come for free. all advanced workflows need a paid lincense. The consistency of the overall workflow is not great either since plugins are made by various people and do not require to follw a consistent pattern (in terms of keybindings, ...).
...
Furthermore I would like to thank @karthink, and @miand for their fabulous suggestions, discussions, contributions, ... in reddit as well as on consult-web repo. This package would not have been possible without @karthink's help in prototyping and ideation and @minad's precious comments and suggestions
